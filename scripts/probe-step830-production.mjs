/**
 * Read-only Production count probe for STEP 8.30.
...
const dest = path.join(root, 'ocr-tests/taxonomy/step8-30')
...
const out = path.join(dest, 'production-readonly.json')
const previous = existsSync(path.join(dest, 'production-before.json'))
writeFileSync(out, JSON.stringify(payload, null, 2), 'utf8')
if (!previous) {
  writeFileSync(path.join(dest, 'production-before.json'), JSON.stringify(payload, null, 2), 'utf8')
}
writeFileSync(path.join(dest, 'production-after.json'), JSON.stringify(payload, null, 2), 'utf8')
console.log(
  `production readonly: queried=${payload.queried} drafts=${payload.drafts} type_auto=${payload.type_auto} figures=${payload.figure_assets}/${payload.figure_links} pipeline=${payload.pipeline_runs}/${payload.pipeline_items} writes=0`,
)
process.exit(0)
