...
let payload
if (!url || !key) {
  payload = snapshot({ queried: false, reason: 'SUPABASE_SERVICE_ROLE_KEY_MISSING' })
} else {
  payload = snapshot({
    queried: true,
    reason: 'read_only',
...
console.log(
  `probe 8.31 read-only drafts=${payload.drafts ?? 'n/a'} needs_review=${payload.needs_review ?? 'n/a'} pipeline_items=${payload.pipeline_items ?? 'n/a'}`,
)
if (!existsSync(path.join(dest, 'production-readonly.json'))) process.exit(1)
process.exit(0)
