/**
 * Read-only Production count probe for STEP 8.31.
...
const dest = path.join(root, 'ocr-tests/taxonomy/step8-31')
...
writeFileSync(path.join(dest, 'production-readonly.json'), JSON.stringify(payload, null, 2))
if (!existsSync(path.join(dest, 'production-before.json'))) {
  writeFileSync(path.join(dest, 'production-before.json'), JSON.stringify(payload, null, 2))
}
console.log(
  `STEP 8.31 probe queried=${payload.queried} drafts=${payload.drafts} AUTO=${payload.type_auto} figures=${payload.figure_assets}/${payload.figure_links} pipeline=${payload.pipeline_runs}/${payload.pipeline_items} needs_review=${payload.needs_review}`,
)
process.exit(0)
