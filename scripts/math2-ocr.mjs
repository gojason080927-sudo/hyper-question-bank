/**
 * 쎈 공통수학 2 OCR only. Default dry-run.
...
const tests = spawnSync(
  process.execPath,
  [path.join(root, 'node_modules/vitest/vitest.mjs'), 'run', 'src/lib/ingestion/math2Ocr.test.ts', '--reporter=dot'],
  { cwd: root, stdio: 'inherit' },
)
if ((tests.status ?? 1) !== 0) process.exit(tests.status ?? 1)

const npx = spawnSync('npx', ['--yes', 'tsx', path.join(root, 'src/lib/ingestion/math2OcrCli.ts'), ...argv], {
  cwd: root,
  stdio: 'inherit',
  env: process.env,
})
