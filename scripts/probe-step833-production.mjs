/**
 * Read-only Production count probe for STEP 8.33.
...
const dest = path.join(root, 'ocr-tests/taxonomy/step8-33')
...
if (!url || !key) {
  ...
  try {
    payload = snapshot({
      ...
      type_auto: await countExact(url, key, 'problem_classification_meta', { classification_status: 'AUTO' }),
      ...
      fingerprints: await countExact(url, key, 'content_fingerprints'),
      ssen_document: SSEN,
    })
  } catch (error) {
    payload = snapshot({
...
console.log(
  `STEP 8.33 production probe queried=${payload.queried} drafts=${payload.drafts} type_auto=${payload.type_auto} figures=${payload.figure_assets}/${payload.figure_links} needs_review=${payload.needs_review} fingerprints=${payload.fingerprints}`,
)
