...
check(tests.includes('IDENTITY_PROPERTY') && tests.includes('나눗셈과 항등식'), 'tests cover remainder-chapter type 05')
...
check(source.includes('ssen-classify-status') || source.includes('HYPER 유형'), 'source detail shows type classification')
check(existsSync(path.join(root, 'scripts/ssen-full-classify.mjs')), 'classify script')
check(readFileSync(path.join(root, 'scripts/ssen-full-classify.mjs'), 'utf8').includes('--complete'), 'classify script has --complete')
const hyper = readFileSync(path.join(root, 'src/lib/taxonomy/ssenHyperComplete.ts'), 'utf8')
const hyperCli = readFileSync(path.join(root, 'src/lib/ingestion/ssenHyperCompleteCli.ts'), 'utf8')
const hyperTests = readFileSync(path.join(root, 'src/lib/taxonomy/ssenHyperComplete.test.ts'), 'utf8')
const ingest = readFileSync(path.join(root, 'src/lib/ingestion/bookCompletePipeline.ts'), 'utf8')
check(hyper.includes('HYPER_COMPLETE_VERSION'), 'hyper complete version')
check(hyper.includes('stripAnswerKeyLeak'), 'leak strip')
check(hyperCli.includes('hqb_apply_auto_clean_text'), 'AUTO_CLEAN leak versions')
check(hyperCli.includes('STUDENT_CARE'), 'hyper CLI refuses student-care')
