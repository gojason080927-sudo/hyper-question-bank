...
| **8.35** | Instructor WYSIWYG problem/math editor + metadata panel + A4 worksheet assembly/print. Lazy OCR→editor JSON on first open. Save = new `TEACHER_EDIT` version. Never `VERIFIED`. See `docs/STEP8_35_WYSIWYG_EDITOR_v1.md`. | Schema + labeled test draft only; never bulk-convert the 1,281 DRAFT stems; never VERIFIED | No paid editor/PDF SaaS; browser print-to-PDF |
| **8.36** | Production instructor stabilization + 쎈 교재 목차 순서 탐색, server pagination, live review queue, fixture hiding, evidence-only OCR/dup repairs. Never `VERIFIED`. Never DELETE. See `docs/STEP8_36_SSEN_OUTLINE_STABILIZE_v1.md`. | Additive outline/list RPCs + SSEN outline mapping + hide exact dups; never VERIFIED | No paid OCR |
