-- Additive fix for STEP 8.5A upsert: qualify bbox so UPDATE is not ambiguous.
...
BEGIN
  ...
  IF existing.problem_id IS NOT NULL THEN
    ...
    IF payload ? 'bbox' THEN
      v_bbox := public.hqb_validate_bbox(payload->'bbox');
      ...
      IF linked_region IS NOT NULL AND NOT public.hqb_region_has_verified_problem(linked_region) THEN
        UPDATE public.source_page_regions AS r
        SET bbox = v_bbox
        WHERE r.id = linked_region AND r.archived_at IS NULL;
      END IF;
      UPDATE public.problem_sources AS ps
      SET bounding_box = v_bbox
      WHERE ps.id = existing.source_id;
    END IF;
