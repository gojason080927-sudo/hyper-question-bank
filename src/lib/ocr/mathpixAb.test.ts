...
import {
  STEP7_DOCUMENT_ID,
  STEP7_GROUND_TRUTH_SHA256_LF,
  STEP7_GROUND_TRUTH_VERSION,
  STEP7_SAMPLE_COUNT,
  STEP7_WINDOWS_OCR_BASELINE,
} from './step7Baseline'
...
describe('corpus hash validation', () => {
  it('freezes the same 26 STEP 7 samples, bboxes, and crop hashes', () => {
    ...
    const rawHash = createHash('sha256').update(truthBytes).digest('hex')
    const lfHash = createHash('sha256').update(truthBytes.toString('utf8').replace(/\r\n/g, '\n')).digest('hex')
    expect(manifest.ground_truth_version).toBe(STEP7_GROUND_TRUTH_VERSION)
    expect(rawHash === STEP7_GROUND_TRUTH_VERSION || lfHash === STEP7_GROUND_TRUTH_SHA256_LF).toBe(true)
