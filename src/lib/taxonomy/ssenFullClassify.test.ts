...
describe('ssen full classify', () => {
  ...
  it('sorts by original problem number so two-column pages keep 쎈 order', () => {
    const planned = planSsenClassify(
      [
        item({ problem_id: 'right', original_problem_number: '0011', source_page: 9, stem: 'A+B를 구하시오', bbox_top: 0.12 }),
        item({ problem_id: 'left', original_problem_number: '0001', source_page: 9, stem: '유형 01 내림차순', bbox_top: 0.18 }),
      ],
      types,
      new Map(),
    )
  ...
  it('uses a unique 유형 pill title and does not inherit into 실력 굳히기', () => {
    ...
    const planned = planSsenClassify(
      [
        item({ problem_id: 'p46', original_problem_number: '0046', source_page: 13, stem: '전개식에서 x의 계수는?', bbox_top: 0.12 }),
        item({ problem_id: 'p51', original_problem_number: '0051', source_page: 13, stem: '전개식에서 x^9의 계수는?', bbox_top: 0.22 }),
        item({ problem_id: 'p52', original_problem_number: '0052', source_page: 13, stem: '다음 중 옳지 않은 것은?', bbox_top: 0.52 }),
        item({ problem_id: 'p97', original_problem_number: '0097', source_page: 20, stem: '직사각형 ABCD의 넓이를 나타내시오', bbox_top: 0.12 }),
      ],
      types,
      new Map(),
      headings,
    )
    expect(planned.decisions[0]?.type_code).toBe('02')
    expect(planned.decisions[1]?.type_code).toBe('02')
    expect(planned.decisions[2]?.type_code).toBe('03')
    expect(planned.decisions[3]?.verdict).toBe('HUMAN')
    expect(planned.decisions[3]?.reasons).toContain('TYPE_MARKER_MISSING')
  })
  ...
  it('maps a unique 기본 다잡기 concept title and clears inherit on a multi-type concept', () => {
    ...
    const planned = planSsenClassify(
      [
        item({ problem_id: 'p1', original_problem_number: '0001', source_page: 9, stem: '내림차순으로 정리하시오', bbox_top: 0.12 }),
        item({ problem_id: 'p13', original_problem_number: '0013', source_page: 9, stem: '다음 식을 전개하시오', bbox_top: 0.5 }),
      ],
      types,
      new Map(),
      headings,
