...
export function PipelineReviewPage() {
  ...
  const filtered = queue.filter((row) => {
    ...
    if (filter === 'HUMAN_REVIEW' || filter === 'BLOCKED' || filter === 'AUTO_APPROVED') return row.status === filter
    return true
  })
  ...
  return (
    <main className="page wide">
      <div className="page-head">
        <div>
          <p className="kicker">STEP 8.31</p>
          <h1>강사 검수 큐</h1>
          <p className="muted">Confidence gate 결과. 자동 확정(VERIFIED)하지 않습니다.</p>
        </div>
      ...
      <div className="filters pipeline-filters">
        ...
        <button
          type="button"
          className={`btn ${filter === 'AUTO_APPROVED' ? 'primary' : 'ghost'}`}
          onClick={() => setFilter('AUTO_APPROVED')}
        >
          AUTO_APPROVED {queue.filter((row) => row.status === 'AUTO_APPROVED').length}
        </button>
      </div>
        ...
        {selected ? (
          <section className="card review-detail">
            ...
            {liveRow ? (
              ...
              <p className="muted">원본 crop은 비교 가능하지만 Production 초안 identity가 없거나 차단입니다.</p>
            )}
            <div className="check-row">
              {Object.entries(CHECK_LABELS).map(([key, label]) => (
                <span
                  key={key}
                  className={`status-pill ${selected.checks[key] === 'PASS' ? 'verified' : 'needs_review'}`}
                >
